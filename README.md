  ...
  return (
    <>
      <nav style={{
        ...
        background: menuOpen ? 'var(--nav-bg)' : scrolled ? 'var(--nav-bg)' : 'transparent',
        ...
        <span onClick={()=>{window.scrollTo({top:0,behavior:'smooth'});setMenuOpen(false);}} style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:17,cursor:'pointer',letterSpacing:'-0.3px',color:'var(--text)',userSelect:'none'}}>
        ...
        {mobile && (
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <button onClick={toggleMode} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text-faint)',padding:4}}>
              {mode==='dark'
            ...
            <button onClick={()=>setMenuOpen(o=>!o)} style={{background:'none',border:'none',cursor:'pointer',color:'var(--text)',padding:4,display:'flex',flexDirection:'column',gap:5,width:24}}>
              <span style={{display:'block',height:1.5,background:'var(--text)',borderRadius:2,transition:'all 0.3s',transform:menuOpen?'rotate(45deg) translateY(6.5px)':'none'}}/>
              <span style={{display:'block',height:1.5,background:'var(--text)',borderRadius:2,transition:'all 0.3s',opacity:menuOpen?0:1}}/>
              <span style={{display:'block',height:1.5,background:'var(--text)',borderRadius:2,transition:'all 0.3s',transform:menuOpen?'rotate(-45deg) translateY(-6.5px)':'none'}}/>
            </button>
          </div>
        )}
      ...
      {mobile && menuOpen && (
        <div style={{
          ...
          background:'rgba(5,5,17,0.97)',backdropFilter:'blur(20px)',
          display:'flex',flexDirection:'column',padding:'40px 28px 40px',
          animation:'fadeUp 0.25s ease both'
        }}>
          <div style={{display:'flex',flexDirection:'column',gap:6,marginBottom:40}}>
            {links.map((k,i)=>(
              <button key={k} onClick={()=>scroll(k)} style={{
                background:'none',border:'none',color:'var(--text)',cursor:'pointer',
                fontFamily:"'Space Grotesk',sans-serif",fontSize:28,fontWeight:700,
                letterSpacing:'-0.8px',padding:'10px 0',textAlign:'left',
                borderBottom:'1px solid var(--border-dim)',
                animation:`fadeUp 0.35s ease ${i*0.05}s both`,
                transition:'color 0.2s'
              }}
              onTouchStart={e=>e.currentTarget.style.color=acc.hex}
          ...
          <div style={{display:'flex',gap:8}}>
            {['fr','en','ko'].map(l=>(
              <button key={l} onClick={()=>{setLang(l);}} style={{
                background:lang===l?acc.hex:'rgba(20,15,40,0.8)',
                color:lang===l?'#08051a':'var(--text-faint)',
                border:`1px solid ${lang===l?acc.hex:'var(--border)'}`,
                borderRadius:8,padding:'8px 18px',cursor:'pointer',
                fontFamily:l==='ko'?"'Noto Sans KR',sans-serif":"'Space Grotesk',sans-serif",
                fontSize:14,fontWeight:lang===l?700:500,transition:'all 0.2s'
...
function Hero({ t, lang, accent, mode }) {
  ...
  return (
    <section id="hero" style={{position:'relative',height:'100vh',display:'flex',alignItems:'center',overflow:'hidden'}}>
      ...
      <div style={{position:'relative',padding:mobile?'0 24px':'0 88px',maxWidth:980}}>
        ...
        <h1 style={{fontFamily:ff,fontSize:mobile?'clamp(40px,11vw,64px)':'clamp(52px,7.5vw,100px)',fontWeight:700,lineHeight:1.04,letterSpacing:mobile?'-1.5px':'-2.5px',color:'var(--text)',marginBottom:18,animation:'fadeUp 0.7s ease both',animationDelay:'0.25s',textShadow:'0 2px 32px rgba(80,40,160,0.18)'}}>
          Niszczota<br/>
        ...
        <div style={{display:'flex',gap:12,flexWrap:'wrap',animation:'fadeUp 0.7s ease both',animationDelay:'0.65s'}}>
          <button ref={mobile?undefined:btn1} onClick={()=>scroll('projects')} style={{background:acc.hex,color:mode==='light'?'#fff':'#08051a',border:'none',borderRadius:10,padding:'13px 30px',fontFamily:"'Space Grotesk',sans-serif",fontSize:14,fontWeight:700,cursor:'pointer',transition:'box-shadow 0.25s,transform 0.18s',letterSpacing:'-0.2px'}}
          ...
          onMouseEnter={e=>{e.currentTarget.style.borderColor=acc.hex;e.currentTarget.style.color=acc.hex;}}
          onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.color='var(--text)';}} style={{background:'var(--surface)',color:'var(--text)',border:'1px solid var(--border)',borderRadius:10,padding: mobile?'13px 24px':'13px 30px',fontFamily:"'Space Grotesk',sans-serif",fontSize:14,fontWeight:500,cursor:'pointer',transition:'all 0.22s',backdropFilter:'blur(8px)'}}>{t.hero.cta2}</button>
        </div>
      </div>
...
function Section({ id, bg, children }) {
  ...
  const mobile = useIsMobile();
  return (
    <section id={id} style={{background:bg||'transparent',padding:mobile?'64px 0':'100px 0'}}>
      <div style={{maxWidth:1160,margin:'0 auto',padding:`0 ${mobile?'20px':'80px'}`}}>{children}</div>
    </section>
  );
}
    ...
    <Section id="music" bg="var(--section-alt)">
      ...
      <div className="reveal" style={{
        ...
        <div style={{padding:'28px 32px 24px',borderBottom:'1px solid var(--border-dim)'}}>
          <div style={{display:'flex',flexDirection:mobile?'column':'row',justifyContent:'space-between',alignItems:mobile?'flex-start':'flex-start',marginBottom:20,gap:mobile?16:0}}>
            <div>
...
function CurrentlyBuilding({ t, lang, accent }) {
  ...
  return (
    <section id="current" style={{padding:mobile?'0 0 56px':'0 0 80px'}}>
      <div style={{maxWidth:1160,margin:'0 auto',padding:mobile?'0 20px':'0 80px'}}>
        <div className="reveal" style={{background:'var(--surface)',border:`1px solid ${acc.hex}33`,borderRadius:16,padding:'28px 32px',backdropFilter:'blur(12px)',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:mobile?'1fr':'1fr auto',gap:mobile?24:32,alignItems:'center'}}>
            <div>
...
function About({ t, lang, accent }) {
  ...
  return (
    <Section id="about">
      ...
      <div style={{display:'grid',gridTemplateColumns:mobile?'1fr':'1fr 1.5fr',gap:mobile?36:80,alignItems:'start'}}>
...
function Footer({ t, accent, onEaster }) {
  ...
  return (
    <footer style={{padding:mobile?'28px 20px':'36px 80px',borderTop:'1px solid var(--border-dim)',display:'flex',flexDirection:mobile?'column':'row',justifyContent:'space-between',alignItems:'center',gap:mobile?12:0,textAlign:mobile?'center':'left'}}>
