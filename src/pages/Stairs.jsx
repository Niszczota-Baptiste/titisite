  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050511', color: '#ede8f8', fontFamily: "'Inter',sans-serif" }}>
      ...
      <main style={{
        ...
        padding: `${mobile ? 18 : 28}px clamp(16px,4vw,32px) 80px`,
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ ...muted, fontSize: mobile ? 13 : 14 }}>
              Catalogue partagé : nb de marches, pair/impair, grandes marches, position GPS.
            </p>
          </div>
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
        ...
        <Link to="/project" style={navLinkStyle}>Projets</Link>
...
function Stats({ items }) {
  ...
  return (
    <section style={{
      ...
      <StatCard label="Total" main={total} sub="escaliers recensés" />
      <StatCard
        label="Pair / Impair"
        main={`${pct(even, total)}%`}
...
function StatCard({ label, main, sub, color }) {
  return (
    <div style={{
      ...
      {sub && (
        <div style={{ fontSize: 12, color: 'rgba(180,170,200,0.6)', marginTop: 4 }}>
          {sub}
        </div>
      )}
    </div>
...
function Filters({
  search, setSearch, country, setCountry, region, setRegion, countries, regions,
}) {
  return (
    <section style={{
      display: 'grid', gap: 10,
      gridTemplateColumns: 'minmax(180px, 1fr) minmax(140px, 200px) minmax(140px, 200px)',
    }}>
      <Input
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 12, color: 'rgba(180,170,200,0.6)', marginTop: 2 }}>
          <strong style={{ color: '#ede8f8' }}>{entry.stepCount}</strong> marches
          {' · '}{entry.isEven ? 'pair' : 'impair'}
          {entry.largeSteps && ' · grandes marches'}
          {entry.stepHeightCm ? ` · ${entry.stepHeightCm} cm` : ''}
          {' · '}{formatDate(entry.encounteredAt)}
          {entry.city && ` · ${entry.city}`}
        </div>
        {entry.notes && (
          <div style={{ fontSize: 12, color: 'rgba(180,170,200,0.55)', marginTop: 4, fontStyle: 'italic' }}>
            {entry.notes}
          </div>
        )}
        {needsGeo && (
          <div style={{
            fontSize: 11, color: BIG_RING, marginTop: 4,
      ...
      <div style={{ display: 'flex', gap: 6, marginLeft: mobile ? 0 : 'auto' }}>
...
function StairForm({ entry, onSaved, onCancel }) {
  const isEdit = !!entry;
    ...
    <div style={modalBackdrop} onClick={onCancel}>
      ...
      >
        ...
        <label style={{
          ...
          cursor: 'pointer', fontSize: 13.5, color: '#ede8f8',
        }}>
          <input
            type="checkbox"
            checked={largeSteps}
        ...
        <div style={{
          ...
          <label style={{
            ...
            cursor: 'pointer', fontSize: 12, color: 'rgba(180,170,200,0.8)',
          }}>
            <input
              type="checkbox"
              checked={manualGeo}
            ...
            />
            Saisir manuellement pays / région / ville (si la résolution auto échoue)
          </label>
          {manualGeo && (
...
const modalBox = {
  ...
  background: '#0b0620', border: `1px solid rgba(${ACC_RGB},0.3)`,
