...
export const api = {
  playlist: {
    ...
    play: (provider, id) => request('POST', `/playlist/play/${encodeURIComponent(provider)}/${encodeURIComponent(id)}`, {}),
    queue: () => request('GET', '/playlist/queue'),
    spotifyPlayback: () => request('GET', '/playlist/playback/spotify'),
    appleStarted: id => request('POST', `/playlist/queue/${encodeURIComponent(id)}/apple/started`, {}),
    queueAdd: (track) => request('POST', '/playlist/queue', track),
    queueClaimApple: (receiver) => request('POST', '/playlist/queue/apple/claim', { receiver }),
    queuePlay: (id) => request('POST', `/playlist/queue/${encodeURIComponent(id)}/play`, {}),
    queueMatch: (id, provider, remote) => request('POST', `/playlist/queue/${encodeURIComponent(id)}/${provider}/match`, { id: remote }),
    queueAppleAck: (id, claim, delivered) => request('POST', `/playlist/queue/${encodeURIComponent(id)}/apple`, { claim, delivered }),
    queueRetry: (id, provider, confirmAbsent) => request('POST', `/playlist/queue/${encodeURIComponent(id)}/retry`, { provider, confirmAbsent }),
    queueRemove: (id) => request('DELETE', `/playlist/queue/${encodeURIComponent(id)}`),
    ...
    setup: (provider) => request('POST', '/playlist/setup', { provider }),
    join: (code) => request('POST', '/playlist/join', { code }),
    invite: () => request('POST', '/playlist/invite', {}),
    settings: (provider, values) => request('PUT', `/playlist/${encodeURIComponent(provider)}/settings`, values),
    disconnect: (provider) => request('POST', `/playlist/${encodeURIComponent(provider)}/disconnect`, {}),
  },
