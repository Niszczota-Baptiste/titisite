  ...
  const load = async () => {
    try {
      const [m, f, u] = await Promise.all([
        api.get('/meetings'), api.get('/features'), api.get('/users'),
      ]);
        ...
        {inMonth && (
          <button
            ...
            style={{
              ...
              color: 'rgba(180,170,200,0.3)', fontSize: 13, padding: 0,
              lineHeight: 1,
            }}
...
function EventChip({ ev, onMeeting, onFeature }) {
  ...
  const due = computeDueForChip(ev.data);
...
function computeDueForChip(feature) {
  if (feature.status === 'done') return 'done';
  const now = Date.now() / 1000;
  if (feature.dueDate < now) return 'overdue';
  return 'upcoming';
}
...
/**
 * Build a 6-row grid starting on Monday for the month containing `cursor`.
 */
function buildMonthGrid(cursor) {
  const year = cursor.getFullYear();
  const month = cursor.getMonth();
  const first = new Date(year, month, 1);
  // Week starts Monday: shift 0=Sun→6, 1=Mon→0
